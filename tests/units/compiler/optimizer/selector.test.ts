...
describe("Optimizer: selector helpers", () => {
    ...
    test("getForBlockSelectorInfos collects selector operations from compiled template", () => {
        ...
        expect(
            infos.some(
                item =>
                    item.operation.method === "setAttribute" && item.operation.attrName === "title"
            )
        ).toBe(true)
        ...
        expect(
            infos.some(
                item =>
                    item.operation.method === "setAttribute" && item.operation.attrName === "value"
            )
        ).toBe(false)
    })
