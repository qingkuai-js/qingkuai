...
function injectCallSignatureDocs(targetDtsPath: string) {
    ...
    const sourceFile = ts.createSourceFile(
        targetDtsPath,
        readSync(targetDtsPath),
        ts.ScriptTarget.Latest,
        true
    )
    const editor = new CodeEditor(content, 0)
    const interfaceToDoc = new Map<string, string>()
    for (const statement of sourceFile.statements) {
        ...
        for (const member of statement.members) {
            ...
            if (doc.length) {
                interfaceToDoc.set(statement.name.text, formatSourceCode(indent + doc[0].getText()))
            }
            break
        }
    }
    ...
    for (const statement of sourceFile.statements) {
        ...
        for (const decl of statement.declarationList.declarations) {
            ...
            const doc = interfaceToDoc.get(decl.type.typeName.getText(sourceFile))
            if (!doc) {
                continue
            }
            ...
            editor.insert(insertPos, leading + "// Generated by build-types script\n" + doc + "\n")
        }
    }
