...
export function transformScript(source: string, indentN = 0) {
    ...
    // 偏移量如果遇到了在标识符前添加__w__前缀且当前索引存在sourcemap信息的情况时，会将对应生成列的偏移量-3
    let mappingLine = 1
    ...
    // 当替换字符是__w__或[__w__标识符前缀时，当前列会记录偏移量-5（前缀固定长度），下一列的额外偏偏移量则为+10
    ...
    for (let i = 0; i < source.length; i++) {
        ...
        // 如果是text为__w__或[__w__前缀时，当前列不需要增加而是等待下一列再增加
        while (replacementItem && replacementItem.index === i) {
            const { text } = replacementItem
            if (isString(text)) {
                if (shouldGenerateSourcemap && /^\[?__w__$/.test(text)) {
                    nextColumnOffset += 5
                    eachColumnOffset[currentColumn] -= 5
                }
